...
def test_mps_fallback_flag_in_device_info_dict():
    from personas.models.device import DeviceInfo
...
def test_enable_mps_fallback_sets_env(monkeypatch):
    from personas.models import device as device_mod
...
def test_apply_chat_template_raw_path():
    from personas.models.generation import apply_chat_template
...
def test_role_keyed_config():
    from personas.configs.schema import Config, ModelConfig
...
def test_data_default_n_items_pilot_sized():
    from personas.configs.schema import DataConfig, EvalConfig
...
def test_validate_layers_rejects_gpt2_layer_12():
    from personas.utils.validation import validate_layers
...
def test_registry_revision_pinned():
    from personas.models.registry import MODEL_REGISTRY, get_model_spec
...
def test_mde_and_tost():
    from personas.evaluation.metrics import Estimate, minimum_detectable_effect, tost_equivalence
...
def test_patch_activations_and_attn_exported():
    from personas.models.hooks import intervene_attention_heads, patch_activations
...
def test_ablation_controls_and_dropout():
    from personas.ablation import run_component_dropout, run_control_arms, run_seed_sweep
...
def test_reporting_roundtrip(tmp_path):
    ...
    from personas.reporting import aggregate_results, write_figures, write_tables
    from personas.utils.run_manifest import write_json_atomic
