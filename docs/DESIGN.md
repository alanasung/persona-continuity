# Design Document: How Sharply Personas Switch Under Soft Steering

## 1. Purpose

This document is the method contract for **How Sharply Personas Switch Under Soft Steering**. It specifies what is
...
One-liner: Test whether persona changes are discrete flips or continuous drifts under steering.

Hypothesis: Test whether persona changes are discrete flips or continuous drifts under steering.
...
|---|---|---|
| Config schema | `personas.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `personas.configs.loader` | Hydra compose API |
| Device | `personas.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `personas.models.registry` | Pinned revisions + hardware fit |
| Hooks | `personas.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `personas.models.generation` | Chat template path recording |
| Data | `personas.data` | Synthetic items, splits, manifests |
| Evaluation | `personas.evaluation` | Metrics, failure harness, protocols |
| Ablation | `personas.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `personas.reporting` | Aggregate → MD/TeX/figures |
| Cache | `personas.cache` | Versioned artifact store |
...
`src/personas/personas/` are preserved across regeneration.
